...
function renderQuestion() {
  ...
  $("#answer-list").replaceChildren(...question.answers.map((ans, i) => {
    ...
    if (state.answers[index] === i) {
      ...
      if (isDarkMode) {
        btn.style.backgroundColor = 'rgb(45, 45, 52)';
      }
    }
...
// ---------------------------------------------------------
// DOM 載入初始化與全域事件代理（徹底解決動態按鈕點擊失效問題）
// ---------------------------------------------------------
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------------------------------------------------------
  // 互動組件事件委派（強制確保動態產生的 Coffee、分享、卡片樣式選單百分百可點擊）
  // ---------------------------------------------------------
  document.addEventListener("click", (e) => {
    // 1. Buy Me a Coffee 贊助按鈕開啟 Modal
    const coffeeBtn = e.target.closest('.coffee-button') || e.target.closest('[data-action="open-donate"]');
    if (coffeeBtn) {
      e.preventDefault();
    ...
    // 2. 贊助 Modal 關閉按鈕
    const closeModalBtn = e.target.closest('.modal-close') || e.target.closest('[data-action="close-donate"]');
    if (closeModalBtn) {
      e.preventDefault();
      const modal = document.querySelector('.modal-overlay');
      if (modal) {
    ...
    // 4. 卡片背景選單切換 (展開/收合)
    const bgToggleBtn = e.target.closest('.card-tools .secondary-button') || e.target.closest('[data-action="toggle-bg-menu"]');
    if (bgToggleBtn) {
      e.preventDefault();
      const toolsContainer = bgToggleBtn.closest('.card-tools');
      const menu = toolsContainer ? toolsContainer.querySelector('.card-bg-menu') : null;
      if (menu) {
    ...
    // 5. 卡片漸變背景選項點擊處理
    const bgOption = e.target.closest('.card-bg-option');
    if (bgOption) {
      e.preventDefault();
      const bgType = bgOption.dataset.bg;
      const card = document.querySelector('.result-card');
      if (card && bgType) {
        card.setAttribute('data-background', bgType);
        const menu = bgOption.closest('.card-bg-menu');
        if (menu) {
          menu.querySelectorAll('.card-bg-option').forEach(opt => opt.classList.remove('is-selected'));
          bgOption.classList.add('is-selected');
          menu.hidden = true;
        }
      }
      return;
    ...
    // 6. 分享選單切換
    const shareToggleBtn = e.target.closest('.share-menu-toggle') || e.target.closest('[data-action="toggle-share"]');
    if (shareToggleBtn) {
      e.preventDefault();
      const wrap = shareToggleBtn.closest('.share-menu-wrap');
      const shareMenu = wrap ? wrap.querySelector('.share-menu') : null;
      if (shareMenu) {
    ...
    // 7. 點擊外部自動關閉下拉選單
    if (!e.target.closest('.card-tools') && !e.target.closest('.share-menu-wrap')) {
      document.querySelectorAll('.card-bg-menu').forEach(menu => { menu.hidden = true; });
...
function manualThemeTransition(dark) {
  ...
  elements.forEach(el => {
    // 嚴格排除按鈕與贊助列，避免動畫過程影響互動與樣式
    if (el.tagName === 'BUTTON' || el.closest('button') || el.closest('.coffee-button') || el.closest('.support-bar') || el.classList.contains('coffee-button')) {
      return;
    }
  ...
  const timer = setInterval(() => {
    ...
    animations.forEach(anim => {
      if (anim.startBg) {
        ...
        const b = Math.round(anim.startBg[2] + (anim.targetBg[2] - anim.startBg[2] * progress)); // 修復顏色計算
